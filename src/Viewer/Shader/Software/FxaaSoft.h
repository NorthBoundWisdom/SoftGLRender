...
#define SUBPIXEL_QUALITY 0.75f
  ...
  glm::vec3 fxaa()
  {
    ...
    float lumaMin =
      glm::min(lumaCenter, glm::min(glm::min(lumaDown, lumaUp), glm::min(lumaLeft, lumaRight)));
    float lumaMax =
      glm::max(lumaCenter, glm::max(glm::max(lumaDown, lumaUp), glm::max(lumaLeft, lumaRight)));
    ...
    glm::vec2 offset =
      isHorizontal ? glm::vec2(inverseScreenSize.x, 0.0f) : glm::vec2(0.0f, inverseScreenSize.y);
    ...
    float lumaAverage =
      (1.0f / 12.0f) * (2.0f * (lumaDownUp + lumaLeftRight) + lumaLeftCorners + lumaRightCorners);
