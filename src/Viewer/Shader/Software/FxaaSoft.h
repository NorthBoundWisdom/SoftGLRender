...
#define QUALITY(q)                                                                                 \
    ((q) < 5.f ? 1.0f : ((q) > 5.f ? ((q) < 10.f ? 2.0f : ((q) < 11.f ? 4.0f : 8.0f)) : 1.5f))
...
#define SUBPIXEL_QUALITY 0.75f
    ...
    glm::vec3 fxaa()
    {
        ...
        float lumaDown =
            rgb2luma(textureLodOffset(u->u_screenTexture, v->v_texCoord, 0.f, glm::ivec2(0, -1)));
        float lumaUp =
            rgb2luma(textureLodOffset(u->u_screenTexture, v->v_texCoord, 0.f, glm::ivec2(0, 1)));
        float lumaLeft =
            rgb2luma(textureLodOffset(u->u_screenTexture, v->v_texCoord, 0.f, glm::ivec2(-1, 0)));
        float lumaRight =
            rgb2luma(textureLodOffset(u->u_screenTexture, v->v_texCoord, 0.f, glm::ivec2(1, 0)));

        // Find the maximum and minimum luma around the current fragment.
        float lumaMin = glm::min(
            lumaCenter, glm::min(glm::min(lumaDown, lumaUp), glm::min(lumaLeft, lumaRight)));
        float lumaMax = glm::max(
            lumaCenter, glm::max(glm::max(lumaDown, lumaUp), glm::max(lumaLeft, lumaRight)));

        // Compute the delta.
        float lumaRange = lumaMax - lumaMin;

        // If the luma variation is lower that a threshold (or if we are in a really dark area), we
        // are not on an edge, don't perform any AA.
        ...
        float lumaDownLeft =
            rgb2luma(textureLodOffset(u->u_screenTexture, v->v_texCoord, 0.f, glm::ivec2(-1, -1)));
        float lumaUpRight =
            rgb2luma(textureLodOffset(u->u_screenTexture, v->v_texCoord, 0.f, glm::ivec2(1, 1)));
        float lumaUpLeft =
            rgb2luma(textureLodOffset(u->u_screenTexture, v->v_texCoord, 0.f, glm::ivec2(-1, 1)));
        ...
        // Combine the four edges lumas (using intermediary variables for future computations with
        // the same values).
        ...
        glm::vec2 offset = isHorizontal ? glm::vec2(inverseScreenSize.x, 0.0f) :
                                          glm::vec2(0.0f, inverseScreenSize.y);
        ...
        // Ratio of the delta between the global average and the center luma, over the luma range in
        // the 3x3 neighborhood.
        float subPixelOffset1 =
            glm::clamp(glm::abs(lumaAverage - lumaCenter) / lumaRange, 0.0f, 1.0f);
        float subPixelOffset2 =
            (-2.0f * subPixelOffset1 + 3.0f) * subPixelOffset1 * subPixelOffset1;
