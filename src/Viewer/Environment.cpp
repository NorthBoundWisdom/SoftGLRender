...
bool IBLGenerator::convertEquirectangular(
    const std::function<bool(ShaderProgram &program)> &shaderFunc,
    const std::shared_ptr<Texture> &texIn, std::shared_ptr<Texture> &texOut)
{
    ...
    bool success =
        createCubeRenderContext(ctx, shaderFunc, std::dynamic_pointer_cast<Texture>(texIn),
    ...
    const std::shared_ptr<Texture> &texIn, std::shared_ptr<Texture> &texOut)
{
    ...
    contextCache_.push_back(std::make_shared<CubeRenderContext>());
    CubeRenderContext &ctx = *contextCache_.back();
    bool success = createCubeRenderContext(
        ctx, shaderFunc, std::dynamic_pointer_cast<Texture>(texIn), MaterialTexType_CUBE);
    if (!success)
    {
        LOGE("create render context failed");
        return false;
...
bool IBLGenerator::generatePrefilterMap(
    const std::function<bool(ShaderProgram &program)> &shaderFunc,
    const std::shared_ptr<Texture> &texIn, std::shared_ptr<Texture> &texOut)
{
    ...
    auto uniformsBlockPrefilter =
        renderer_->createUniformBlock("UniformsPrefilter", sizeof(UniformsIBLPrefilter));
    ctx.modelSkybox.material->materialObj->shaderResources->blocks[UniformBlock_IBLPrefilter] =
        uniformsBlockPrefilter;
    ...
    {
        drawCubeFaces(
            ctx, texOut->getLevelWidth(level), texOut->getLevelHeight(level), texOut, level,
            [&]() -> void
            {
...
bool IBLGenerator::createCubeRenderContext(
    CubeRenderContext &ctx, const std::function<bool(ShaderProgram &program)> &shaderFunc,
    const std::shared_ptr<Texture> &texIn, MaterialTexType texType)
{
    ...
    ctx.modelSkybox.material->materialObj->shaderResources->blocks[UniformBlock_Model] =
        ctx.uniformsBlockModel;
...
void IBLGenerator::drawCubeFaces(CubeRenderContext &ctx, uint32_t width, uint32_t height,
                                 std::shared_ptr<Texture> &texOut, uint32_t texOutLevel,
                                 const std::function<void()> &beforeDraw)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        uniformsModel.u_modelViewProjectionMatrix =
            ctx.camera.projectionMatrix() * viewMatrix * modelMatrix;
        ...
        ctx.fbo->setColorAttachment(texOut, CubeMapFace(TEXTURE_CUBE_MAP_POSITIVE_X + i),
                                    texOutLevel);
...
{
    return HashUtils::getHashMD5(tex->tag + std::to_string(tex->width) +
