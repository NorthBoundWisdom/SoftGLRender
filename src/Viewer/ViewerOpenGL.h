...
{
  public:
  ViewerOpenGL(Config &config, Camera &camera)
  ...
  int swapBuffer() override
  {
    ...
    if (texColorMain_->multiSample)
    {
      ...
      GL_CHECK(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                      GL_TEXTURE_2D_MULTISAMPLE, texColorMain_->getId(), 0));

      GL_CHECK(glBindTexture(GL_TEXTURE_2D, outTexId_));
      GL_CHECK(glBindFramebuffer(GL_FRAMEBUFFER, fbo_out_));
      GL_CHECK(
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, outTexId_, 0));

      GL_CHECK(glBindFramebuffer(GL_READ_FRAMEBUFFER, fbo_in_));
      GL_CHECK(glBindFramebuffer(GL_DRAW_FRAMEBUFFER, fbo_out_));

      GL_CHECK(glBlitFramebuffer(0, 0, width, height, 0, 0, width, height, GL_COLOR_BUFFER_BIT,
                                 GL_NEAREST));
      return outTexId_;
    }
