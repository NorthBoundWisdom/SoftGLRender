...
class VertexArrayObjectOpenGL : public VertexArrayObject
{
  ...
  explicit VertexArrayObjectOpenGL(const VertexArray &vertexArr)
  {
    ...
    GL_CHECK(glBufferData(GL_ARRAY_BUFFER, vertexArr.vertexesBufferLength, vertexArr.vertexesBuffer,
                          GL_STATIC_DRAW));
    ...
    for (int i = 0; i < vertexArr.vertexesDesc.size(); i++)
    {
      ...
      GL_CHECK(
        glVertexAttribPointer(i, desc.size, GL_FLOAT, GL_FALSE, desc.stride, (void *)desc.offset));
