...
public:
    explicit VertexArrayObjectOpenGL(const VertexArray &vertexArr)
    {
        ...
        GL_CHECK(glBufferData(GL_ARRAY_BUFFER, vertexArr.vertexesBufferLength,
                              vertexArr.vertexesBuffer, GL_STATIC_DRAW));

        for (int i = 0; i < vertexArr.vertexesDesc.size(); i++)
        {
            auto &desc = vertexArr.vertexesDesc[i];
            GL_CHECK(glVertexAttribPointer(i, desc.size, GL_FLOAT, GL_FALSE, desc.stride,
                                           (void *)desc.offset));
        ...
        GL_CHECK(glBufferData(GL_ELEMENT_ARRAY_BUFFER, vertexArr.indexBufferLength,
                              vertexArr.indexBuffer, GL_STATIC_DRAW));
    }
