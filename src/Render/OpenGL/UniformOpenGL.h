...
#define BIND_TEX_OPENGL(n)                                                                         \
  case n: GL_CHECK(glActiveTexture(GL_TEXTURE##n)); break;
...
class UniformSamplerOpenGL : public UniformSampler
{
  ...
  void setTexture(const std::shared_ptr<Texture> &tex) override
  {
    switch (tex->type)
    {
    case TextureType_2D: texTarget_ = GL_TEXTURE_2D; break;
    case TextureType_CUBE: texTarget_ = GL_TEXTURE_CUBE_MAP; break;
    default: LOGE("UniformSampler::setTexture error: texture type not support"); break;
    }
    texId_ = tex->getId();
  }
