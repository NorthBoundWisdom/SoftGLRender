...
public:
    ...
    void dumpImage(const char *path, uint32_t layer, uint32_t level) override
    {
        ...
        GLenum attachment =
            format == TextureFormat_FLOAT32 ? GL_DEPTH_ATTACHMENT : GL_COLOR_ATTACHMENT0;
        ...
        if (format == TextureFormat_FLOAT32)
        {
            ImageUtils::convertFloatImage(reinterpret_cast<RGBA *>(pixels),
                                          reinterpret_cast<float *>(pixels), levelWidth,
                                          levelHeight);
        }
...
public:
    ...
    void setSamplerDesc(SamplerDesc &sampler) override
    {
        ...
        GL_CHECK(
            glTexParameteri(target_, GL_TEXTURE_MIN_FILTER, OpenGL::cvtFilter(sampler.filterMin)));
        GL_CHECK(
            glTexParameteri(target_, GL_TEXTURE_MAG_FILTER, OpenGL::cvtFilter(sampler.filterMag)));
    ...
    void setImageData(const std::vector<std::shared_ptr<Buffer<RGBA>>> &buffers) override
    {
        ...
        GL_CHECK(glBindTexture(target_, texId_));
        GL_CHECK(glTexImage2D(target_, 0, glDesc_.internalformat, width, height, 0, glDesc_.format,
                              glDesc_.type, buffers[0]->getRawDataPtr()));
    ...
    void initImageData() override
    {
        ...
        if (multiSample)
        {
            GL_CHECK(glTexImage2DMultisample(target_, 4, glDesc_.internalformat, width, height,
                                             GL_TRUE));
        }
        else
        {
            GL_CHECK(glTexImage2D(target_, 0, glDesc_.internalformat, width, height, 0,
...
public:
    ...
    void setImageData(const std::vector<std::shared_ptr<Buffer<RGBA>>> &buffers) override
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            GL_CHECK(glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, glDesc_.internalformat,
                                  width, height, 0, glDesc_.format, glDesc_.type,
    ...
    void initImageData() override
    {
        ...
        {
            GL_CHECK(glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, glDesc_.internalformat,
                                  width, height, 0, glDesc_.format, glDesc_.type, nullptr));
        }
