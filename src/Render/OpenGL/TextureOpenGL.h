  ...
  void dumpImage(const char *path, uint32_t layer, uint32_t level) override
  {
    ...
    if (format == TextureFormat_FLOAT32)
    {
      ImageUtils::convertFloatImage(reinterpret_cast<RGBA *>(pixels),
                                    reinterpret_cast<float *>(pixels), levelWidth, levelHeight);
    }
...
class TextureCubeOpenGL : public TextureOpenGL
{
  ...
  void setSamplerDesc(SamplerDesc &sampler) override
  {
    ...
    GL_CHECK(
      glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, OpenGL::cvtWrap(sampler.wrapS)));
    GL_CHECK(
      glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, OpenGL::cvtWrap(sampler.wrapT)));
    GL_CHECK(
      glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, OpenGL::cvtWrap(sampler.wrapR)));
  ...
  void setImageData(const std::vector<std::shared_ptr<Buffer<RGBA>>> &buffers) override
  {
    ...
    for (int i = 0; i < 6; i++)
    {
      GL_CHECK(glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, glDesc_.internalformat, width,
                            height, 0, glDesc_.format, glDesc_.type, buffers[i]->getRawDataPtr()));
    }
    if (useMipmaps)
    {
  ...
  void initImageData() override
  {
    ...
    {
      GL_CHECK(glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, glDesc_.internalformat, width,
                            height, 0, glDesc_.format, glDesc_.type, nullptr));
    }
