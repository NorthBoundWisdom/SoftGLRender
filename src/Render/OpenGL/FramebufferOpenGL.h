...
public:
    ...
    void setColorAttachment(std::shared_ptr<Texture> &color, int level) override
    {
        ...
        GL_CHECK(glFramebufferTexture2D(
            ...
            color->multiSample ? GL_TEXTURE_2D_MULTISAMPLE : GL_TEXTURE_2D, color->getId(), level));
    }
    ...
    void setColorAttachment(std::shared_ptr<Texture> &color, CubeMapFace face, int level) override
    {
        ...
        GL_CHECK(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                        OpenGL::cvtCubeFace(face), color->getId(), level));
    }
    ...
    void setDepthAttachment(std::shared_ptr<Texture> &depth) override
    {
        ...
        GL_CHECK(glFramebufferTexture2D(
            ...
            depth->multiSample ? GL_TEXTURE_2D_MULTISAMPLE : GL_TEXTURE_2D, depth->getId(), 0));
    }
