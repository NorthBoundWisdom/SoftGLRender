...
#define VK_STRUCTURE_TYPE_SEMAPHORE_GET_HANDLE_INFO                                                \
...
{
  public:
  explicit VKGLInterop(VKContext &ctx)
    : vkCtx_(ctx)
  {
    device_ = ctx.device();
  }
  ...
  static inline const void *getExtImageCreateInfo()
  {
    return &extMemoryImageCreateInfo_;
  }
  static inline const void *getExtMemoryAllocateInfo()
  {
  ...
  void setGLTexture2DStorage(GLuint texture, GLsizei levels, GLenum internalFormat, GLsizei width,
                             GLsizei height);
  ...
  static inline void setVkExtensionsAvailable(bool available)
  {
    vkExtensionsAvailable_ = available;
  };
  static inline bool isAvailable()
  {
    return vkExtensionsAvailable_ && functionsAvailable_;
  }

  static inline const std::vector<const char *> &getRequiredInstanceExtensions()
  {
