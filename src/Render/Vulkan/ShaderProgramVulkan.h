...
public:
    ...
    void endBindUniforms()
    {
        ...
        {
            vkUpdateDescriptorSets(device_, writeDescriptorSets_.size(),
...
private:
        ...
        std::unordered_map<std::string, ShaderUniformDesc> &uniformsDescFragment)
    {
        ...
        VK_CHECK(
            vkCreateDescriptorSetLayout(device_, &layoutInfo, nullptr, &descriptorSetLayouts_[0]));
    }
    ...
    void createDescriptorPool()
    {
        ...
        if (uniformBlockCnt > 0)
        {
            poolSizes.push_back({VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                                 uniformBlockCnt * DESCRIPTOR_SET_POOL_MAX_SIZE});
        }
