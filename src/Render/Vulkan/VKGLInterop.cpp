...
void VKGLInterop::checkFunctionsAvailable()
{
    ...
    {
        LOGE(
            "VKGLInterop::checkFunctionsAvailable failed: gl extern semaphore functions not found");
        return;
    }
...
void VKGLInterop::signalGLComplete()
{
    ...
    GL_CHECK(glSignalSemaphoreEXT(glComplete_.glRef, 0, nullptr, 1,
                                  &sharedMemory_.glAttachedTexture, &dstLayout));
    GL_CHECK(glFlush());
}
