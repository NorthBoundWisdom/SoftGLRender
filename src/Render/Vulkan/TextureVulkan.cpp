    ...
    , glInterop_({ctx})
{
    ...
    if (needResolve_)
    {
        createImage();
        // try lazy allocation
        bool memoryReady = vkCtx_.createImageMemory(
            image_, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT | VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT);
        if (!memoryReady)
        {
...
void TextureVulkan::dumpImage(const char *path, uint32_t layer, uint32_t level)
{
    ...
    readPixels(layer, level,
               [&](uint8_t *buffer, uint32_t w, uint32_t h, uint32_t rowStride) -> void
               {
                   ...
                   {
                       memcpy(pixels + i * rowStride, buffer + i * rowStride,
                              w * getPixelByteSize());
                   }
                                                        ...
                                                        uint32_t height, uint32_t rowStride)> &func)
{
    ...
    {
        transitionImageLayout(
            copyCmd->cmdBuffer, image_.image, subRange, VK_ACCESS_TRANSFER_READ_BIT,
...
void TextureVulkan::generateMipmaps()
{
    ...
    for (uint32_t layer = 0; layer < layerCount_; layer++)
    {
        ...
        for (uint32_t level = 1; level < levelCount_; level++)
        {
            ...
            blit.srcOffsets[1] = {(int32_t)getLevelWidth(level - 1),
                                  (int32_t)getLevelHeight(level - 1), 1};
            ...
            transitionImageLayout(
                cmd->cmdBuffer, image_.image, subRange, VK_ACCESS_TRANSFER_READ_BIT,
                                         ...
                                         VkDeviceSize imageSize)
{
    ...
    transitionImageLayout(copyCmd->cmdBuffer, image_.image, subRange, 0,
                          VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                          VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_PIPELINE_STAGE_TRANSFER_BIT,
                          VK_PIPELINE_STAGE_TRANSFER_BIT);
    ...
    {
        transitionImageLayout(
            copyCmd->cmdBuffer, image_.image, subRange, VK_ACCESS_TRANSFER_WRITE_BIT,
            VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_PIPELINE_STAGE_TRANSFER_BIT,
            VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
    }
