...
{
  public:
  UniformBlockSoft(const std::string &name, int size)
...
class UniformSamplerSoft : public UniformSampler
{
    ...
    : UniformSampler(name, type, format)
  {
    ...
    case TextureType_2D:
      switch (format)
      {
      case TextureFormat_RGBA8: sampler_ = std::make_shared<Sampler2DSoft<RGBA>>(); break;
      case TextureFormat_FLOAT32: sampler_ = std::make_shared<Sampler2DSoft<float>>(); break;
      }
      break;
    case TextureType_CUBE:
      switch (format)
      {
      case TextureFormat_RGBA8: sampler_ = std::make_shared<SamplerCubeSoft<RGBA>>(); break;
      case TextureFormat_FLOAT32: sampler_ = std::make_shared<SamplerCubeSoft<float>>(); break;
      }
      break;
    default: sampler_ = nullptr; break;
    }
  }
