...
template <typename T>
T calcBlendFactor(const T &src, const float &srcAlpha, const T &dst, const float &dstAlpha,
...
glm::vec4 calcBlendColor(glm::vec4 &src, glm::vec4 &dst, const BlendParameters &params)
{
    ...
    auto retAlpha =
        calcBlendFunc<float>(src.a * srcAlphaF, dst.a * dstAlphaF, params.blendFuncAlpha);
