...
public:
    ...
    T textureImpl(TextureImageSoft<T> *tex, glm::vec2 &uv, float lod = 0.f,
...
void BaseSampler<T>::generateMipmaps(TextureImageSoft<T> *tex, bool sample)
{
    ...
    {
        tex->levels.push_back(std::make_shared<ImageBufferSoft<T>>(std::max(1, width >> level),
    ...
    {
        BaseSampler<T>::sampleBufferBilinear(tex->levels[i]->buffer.get(),
                                             tex->levels[i - 1]->buffer.get(), T(0));
    }
}
...
T BaseSampler<T>::textureImpl(TextureImageSoft<T> *tex, glm::vec2 &uv, float lod, glm::ivec2 offset)
{
    if (tex != nullptr && !tex->empty())
    {
        ...
        {
            return sampleBilinear(tex->levels[0]->buffer.get(), uv, wrapMode_, offset,
                                  borderColor_);
        }
        ...
        if (filterMode_ == Filter_NEAREST_MIPMAP_NEAREST ||
            filterMode_ == Filter_LINEAR_MIPMAP_NEAREST)
        {
            int level = glm::clamp((int)glm::ceil(lod + 0.5f) - 1, 0, max_level);
            if (filterMode_ == Filter_NEAREST_MIPMAP_NEAREST)
            {
                return sampleNearest(tex->levels[level]->buffer.get(), uv, wrapMode_, offset,
                                     borderColor_);
            }
            else
            {
                return sampleBilinear(tex->levels[level]->buffer.get(), uv, wrapMode_, offset,
                                      borderColor_);
            }
        }

        if (filterMode_ == Filter_NEAREST_MIPMAP_LINEAR ||
            filterMode_ == Filter_LINEAR_MIPMAP_LINEAR)
        {
            ...
            if (filterMode_ == Filter_NEAREST_MIPMAP_LINEAR)
            {
                texel_hi = sampleNearest(tex->levels[level_hi]->buffer.get(), uv, wrapMode_, offset,
                                         borderColor_);
            }
            else
            {
                texel_hi = sampleBilinear(tex->levels[level_hi]->buffer.get(), uv, wrapMode_,
                                          offset, borderColor_);
            }
            ...
            else
            {
                if (filterMode_ == Filter_NEAREST_MIPMAP_LINEAR)
                {
                    texel_lo = sampleNearest(tex->levels[level_lo]->buffer.get(), uv, wrapMode_,
                                             offset, borderColor_);
                }
                else
                {
                    texel_lo = sampleBilinear(tex->levels[level_lo]->buffer.get(), uv, wrapMode_,
                                              offset, borderColor_);
                }
            }
...
template <typename T>
T BaseSampler<T>::sampleNearest(Buffer<T> *buffer, glm::vec2 &uv, WrapMode wrap, glm::ivec2 &offset,
                                T border)
{
...
template <typename T>
T BaseSampler<T>::sampleBilinear(Buffer<T> *buffer, glm::vec2 &uv, WrapMode wrap,
                                 glm::ivec2 &offset, T border)
{
    glm::vec2 texUV = uv * glm::vec2(buffer->getWidth(), buffer->getHeight());
