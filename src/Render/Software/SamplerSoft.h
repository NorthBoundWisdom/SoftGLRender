...
T BaseSampler<T>::textureImpl(TextureImageSoft<T> *tex, glm::vec2 &uv, float lod, glm::ivec2 offset)
{
  if (tex != nullptr && !tex->empty())
  {
    ...
    if (filterMode_ == Filter_NEAREST_MIPMAP_LINEAR || filterMode_ == Filter_LINEAR_MIPMAP_LINEAR)
    {
      ...
      {
        texel_hi =
          sampleNearest(tex->levels[level_hi]->buffer.get(), uv, wrapMode_, offset, borderColor_);
      }
      else
      {
        texel_hi =
          sampleBilinear(tex->levels[level_hi]->buffer.get(), uv, wrapMode_, offset, borderColor_);
      }
      ...
      else
      {
        ...
        {
          texel_lo =
            sampleNearest(tex->levels[level_lo]->buffer.get(), uv, wrapMode_, offset, borderColor_);
        }
        else
        {
          texel_lo = sampleBilinear(tex->levels[level_lo]->buffer.get(), uv, wrapMode_, offset,
                                    borderColor_);
        }
      }
